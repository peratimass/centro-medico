...
'Ultima Actualizacion: 17-09-2019    11:28 am
...
reintentar:
reintentar2:
    If MsgBox("Linea de Internet Inestable." + "Desea Intentar Conectarse Nuevamente", vbQuestion + vbYesNo, KEY_VENDEDOR) = vbYes Then
        GoTo intentar
    End If
...
Public Sub ConfiguraRstVersion(ByVal strCadena As String)
On Error GoTo salir
reiniciar:
  Set rstVersion = New ADODB.Recordset
  rstVersion.CursorLocation = adUseClient
  rstVersion.Open strCadena, CnBd, adOpenKeyset, adLockOptimistic
  rstVersion.ActiveConnection = Nothing
  Exit Sub
salir:
...
Public Sub ConfiguraRstIN(ByVal strCadena As String)
On Error GoTo salir
reiniciar:
  Set rstIN = New ADODB.Recordset
  rstIN.CursorLocation = adUseClient
  rstIN.Open strCadena, CnBd, adOpenKeyset, adLockOptimistic
  rstIN.ActiveConnection = Nothing
...
Public Function ValorItem(ByVal Valor As Double) As String
    If Valor <> 0 Then
        If KEY_RUC = "10419736444" Or KEY_RUC = "20600549490" Then
            ValorItem = Format(Valor, "#,##0.00000")
        Else
...
Public Function ValorItem_v(ByVal Valor As Double) As String
        ...
        If KEY_RUC = "10419736444" Or KEY_RUC = "20600549490" Then
            ValorItem_v = Format(Valor, "#,##0.00000")
        Else
...
Public Function get_extrangero(ByVal in_dni As String) As String

strCadena = "SELECT extranjero FROM persona WHERE dni='" & Trim(in_dni) & "' LIMIT 1"
Call ConfiguraRstL(strCadena)
If rstL.RecordCount > 0 Then
   get_extrangero = rstL("extranjero")
End If
