...
Public Sub DrawBarcode(ByVal bc_string As String, obj As Control, Nombre As String, ByVal precio As Single)
    ...
    'obj.Print "2.60" + "-" & bc_string
    
    
    y1 = obj.ScaleTop
    y2 = obj.ScaleTop + obj.ScaleHeight - 1.5 * th - 5
    ...
    obj.CurrentX = 3 '(obj.ScaleWidth - tw) / 4
    obj.CurrentY = y2 + 0.1 * th
    'obj.Print "S/" + Format(precio, "#,##0.00") + Space(2) + "-" + bc_string '+ Chr(13) + Mid(Trim(Nombre), 1, 10)
    obj.Print bc_string + Space(2) + Mid(Trim(Nombre), 1, 10)
    
    'Obj.Print "1111111111111111111111" + Chr(13) + "CODIGO"
    'Obj.Print "S/.2.60" + Space(2) & "-" & Space(2) & bc_string
    'obj.Print "NOMBRE"
    'copy to clipboard
